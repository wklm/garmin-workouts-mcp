...
class TestMCPIntegration:
    ...
    async def test_mcp_tools_registered(self):
        ...
        assert expected_tools == registered_tool_names, (
            f"Tool sets don't match. Expected: {expected_tools}, Found: {registered_tool_names}"
        )
    ...
    def test_login_integration_new_login(
        self, mock_garth_login, mock_save, mock_resume
    ):
    ...
    def test_login_integration_login_failure(
        self, mock_logger, mock_exit, mock_garth_login, mock_resume
    ):
    ...
    def test_login_integration_no_credentials_raises_error(self, mock_resume):
        ...
        with pytest.raises(
            ValueError,
            match=r"Garmin email and password must be provided via environment variables \(GARMIN_EMAIL, GARMIN_PASSWORD\).",
        ):
            login()
