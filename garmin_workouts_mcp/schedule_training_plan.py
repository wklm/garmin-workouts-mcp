...
class GarminWorkoutScheduler:
    ...
    def login(self):
        ...
        try:
            ...
            if not email or not password:
                ...
                self.console.print("Please set GARMIN_EMAIL and GARMIN_PASSWORD environment variables.")
                sys.exit(1)
    ...
    def schedule_workout(self, workout_id: str, schedule_date: date, planned_workout: WorkoutData = None) -> Tuple[Optional[str], bool]:
        """
        ...
        try:
            ...
            if calendar_data:
                for item in calendar_data.get("calendarItems", []):
                    if item.get("itemType") == "workout" and item.get("date") == schedule_date.isoformat():
                        ...
                        if existing_workout_id and planned_workout:
                            ...
                            existing_workout = self.get_workout_details(existing_workout_id)
                            if existing_workout and self.workouts_match(existing_workout, planned_workout):
                                # Workouts match - skip scheduling
                                self.console.print(f"[dim]Workout already exists on {schedule_date} and matches - skipping[/dim]")
                                return (existing_schedule_id, True)
                        
                        # Workouts don't match or we couldn't compare - remove existing
                        if existing_schedule_id:
                            self.console.print(f"[yellow]Removing non-matching workout on {schedule_date}[/yellow]")
    ...
    def workouts_match(self, existing_workout: Dict[str, Any], planned_workout: WorkoutData) -> bool:
        """Compare if existing workout matches the planned workout structure."""
        try:
            # Compare workout type
            if existing_workout.get("sportType", {}).get("sportTypeKey") != planned_workout.type:
                return False
    ...
    async def schedule_session(self, session: TrainingSession, retry_count: int = 3) -> ScheduleResult:
        ...
        for attempt in range(retry_count):
            try:
                ...
                workout_data = parse_workout_description(session.garmin_mcp_description, session.session)
                if not workout_data:
                    return ScheduleResult(
                        date=session.date,
                        session_name=session.session,
                ...
                if not self.dry_run:
                    ...
                    if calendar_data:
                        for item in calendar_data.get("calendarItems", []):
                            if item.get("itemType") == "workout" and item.get("date") == session.date.isoformat():
                                existing_workout_id = item.get("workoutId")
                                if existing_workout_id:
                                    existing_workout = self.get_workout_details(existing_workout_id)
                                    if existing_workout and self.workouts_match(existing_workout, workout_data):
                ...
                schedule_id, was_skipped = self.schedule_workout(workout_id, session.date, workout_data)
                ...
                if attempt < retry_count - 1:
                    ...
                    logger.warning(f"Attempt {attempt + 1} failed for {session.date}: {e}. Retrying in {wait_time}s...")
            ...
            for session in plan.all_sessions:
                ...
                if result.success:
                    if result.error == "Rest day - skipped":
                        self.console.print(f"[dim]Skipped {session.date}: Rest day[/dim]")
                    elif result.skipped:
                        self.console.print(f"[cyan]⟳ Exists {session.date}: {session.session} (matched)[/cyan]")
                    else:
                        self.console.print(f"[green]✓ Scheduled {session.date}: {session.session}[/green]")
                else:
                    self.console.print(f"[red]✗ Failed {session.date}: {result.error}[/red]")
    ...
    async def validate_scheduled_workouts(self, plan: TrainingPlan, results: List[ScheduleResult]) -> List[ScheduleResult]:
        ...
        with Progress(
            ...
            for i, session in enumerate(plan.all_sessions):
                ...
                if validation_status == "Valid":
                    self.console.print(f"[green]✓ Validated {session.date}: {session.session}[/green]")
                else:
                    self.console.print(f"[red]✗ Validation failed {session.date}: {validation_status}[/red]")
...
@click.option('--start-date', type=click.DateTime(formats=["%Y-%m-%d"]), help='Override start date')
@click.option('--verbose', '-v', is_flag=True, help='Enable verbose logging')
def main(training_plan_file: str, dry_run: bool, start_date: Optional[datetime], verbose: bool):
    ...
    console.print(Panel(f"[bold blue]Marathon Training Plan Scheduler[/bold blue]\n\nReading: {plan_path.name}"))
    ...
    if not dry_run:
        if not click.confirm("Do you want to schedule these workouts to Garmin Connect?"):
    ...
    try:
        ...
        if not dry_run:
            results = loop.run_until_complete(scheduler.validate_scheduled_workouts(plan, results))
    finally:
        loop.close()
    
    # Display results summary
    successful = sum(1 for r in results if r.success and not r.skipped)
    failed = sum(1 for r in results if not r.success and r.error != "Rest day - skipped")
    rest_days = sum(1 for r in results if r.error == "Rest day - skipped")
    skipped_existing = sum(1 for r in results if r.skipped)
    
    # Validation summary
    validated = sum(1 for r in results if r.validation_status == "Valid" or r.validation_status == "Valid (dry-run)")
    validation_failed = sum(1 for r in results if r.validation_status and "Valid" not in r.validation_status and "Rest day" not in r.validation_status)
