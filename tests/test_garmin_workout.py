...
def test_process_regular_step_missing_duration():
    with pytest.raises(
        ValueError, match="Invalid or missing stepDuration for step: Unnamed Step"
    ):
...
def test_process_repeat_step_missing_iterations():
    with pytest.raises(
        ValueError, match="Invalid or missing numberOfIterations for repeat step."
    ):
    ...
    workout_data = {
        ...
        "steps": [
            {"stepType": "warmup", "stepDuration": 600},
            {
                "stepType": "run",
                "stepDuration": 1800,
                "target": {"type": "pace", "value": [6.0, 7.0], "unit": "min_per_km"},
            },
    ...
    workout_data = {
        ...
        "steps": [
            {"stepType": "warmup", "stepDuration": 900},
            {
                "stepType": "bike",
                "stepDuration": 3600,
                "target": {"type": "power", "value": [150, 200]},
            },
...
def test_calculate_steps_duration_mixed_steps():
    ...
    expected_duration = (
        300 + int(500 * (2 / (2.77 + 3.33))) + 2 * (60 + int(100 * (2 / (2.77 + 3.33))))
    )
